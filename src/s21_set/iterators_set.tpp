...
template <typename key_type, typename Comp>
Iterator<key_type, Comp>::Iterator(const Iterator<key_type, Comp>& other)
...
template <typename key_type, typename Comp>
ConstIterator<key_type, Comp>::ConstIterator(const Node<key_type>* node)
    : current_node(node) {}
...
template <typename key_type, typename Comp>
bool ConstIterator<key_type, Comp>::operator<=(
    ConstIterator<key_type, Comp> node) const {
  if (current_node && node.current_node)
    return ((Comp{}(current_node->key, node.current_node->key)) ||
            (current_node->key == node.current_node->key));
  return 0;
