...
template <typename K,typename Comp>
multiset<K,Comp>& multiset<K,Comp>::operator=(multiset<K,Comp> &&m) noexcept {
...
template <typename K,typename Comp>
std::pair< typename multiset<K,Comp>::iterator, bool> multiset<K,Comp>::insert(const value_type& value, Node<K>* current_node, int assign) {
             Node<K>* temp;
    ...
    if ((Comp{}  (value,current_node->key)) && value != current_node->key) {
        if (current_node->left == set<K,Comp>::leaf_tree) {
            current_node->left = new Node<K>(value, current_node, set<K,Comp>::leaf_tree);
            set<K,Comp>::insertBalanceTree(current_node->left);
            if (set<K,Comp>::begin_node->key > value) set<K,Comp>::begin_node = current_node->left;
        ...
        if (current_node->right == set<K,Comp>::leaf_tree) {
            current_node->right = new Node<K>(value, current_node, set<K,Comp>::leaf_tree);
            set<K,Comp>::insertBalanceTree(current_node->right);
            if (set<K,Comp>::end_node->key < value) set<K,Comp>::end_node = current_node->right;
...
template <typename K, typename Comp>
std::pair<typename set<K,Comp>::iterator, bool> multiset<K,Comp>::insert(const value_type& value)  {
    ++(set<K,Comp>::size_set);
...
template <typename K,typename Comp>
template <class... Args>
std::vector<std::pair<typename set<K,Comp>::iterator, bool>> multiset<K,Comp>::insert_many(Args&&... args)  { 
    std::vector<std::pair<typename set<K,Comp>::iterator, bool>> results;
    multiset<K,Comp> temp = {args...};
    for (auto it = temp.set<K,Comp>::begin(); it <= temp.set<K,Comp>::end(); ++it) { 
...
template <typename K,typename Comp>
typename multiset<K,Comp>::iterator multiset<K,Comp>::lower_bound(const K& key) { 
...
template <typename K,typename Comp>
typename multiset<K,Comp>::iterator multiset<K,Comp>::upper_bound(const K& key) { 
...
template <typename K,typename Comp>
std::pair<typename multiset<K,Comp>::iterator,typename multiset<K,Comp>::iterator> multiset<K,Comp>::equal_range(const K& key) {
